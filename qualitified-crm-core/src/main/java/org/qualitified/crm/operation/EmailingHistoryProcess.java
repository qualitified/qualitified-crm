...
public class EmailingHistoryProcess {
    ...
    public void run() throws OperationException, LoginException, JSONException, InterruptedException {
        ...
        String sendMailCommandId = fetchMailHistoryBulkService.submit(fetchMailHistoryCommand);

        // await end of computation
        fetchMailHistoryBulkService.await(sendMailCommandId, Duration.ofMinutes(1));

        // get status
        BulkStatus fetchMailHistoryBulkStatus = fetchMailHistoryBulkService.getStatus(sendMailCommandId);
