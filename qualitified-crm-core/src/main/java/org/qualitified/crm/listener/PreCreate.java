...
public class PreCreate implements EventListener {

    private Log logger = LogFactory.getLog(PreCreate.class);
    ...
    public void handleEvent(Event event) {
        ...
        if(destinationRef != null && destinationRef.toString().startsWith(adminPath)){
            logger.warn("Document is in config path ["+adminPath+"]. We won't run PreCreate.");
        ...
        for(DocumentModel script : scripts) {
            ...
            try {
                Object obj = automationService.run(operationContext, "javascript.preCreate", params);
                if(obj instanceof DocumentModel){
                    doc.copyContent((DocumentModel)obj);
                }
            } catch (OperationException e) {
                logger.error("Error while running automation script javascript.preCreate", e);
            }
